...
def findLevels(A, level, mode='rising', boxWidth=0, rangeSubset=None):
    ...
    for point in range(len(A)-1):
        thisPointValue, nextPointValue =  A[point], A[point+1]

        if thisPointValue < level and nextPointValue >= level:
            risingInterpFunction = interp1d(np.array([thisPointValue, nextPointValue]), np.array([point, point+1]))
            interpPosition = risingInterpFunction(level)

            rising.append(interpPosition)
            both.append(interpPosition)
            
        if thisPointValue > level and nextPointValue <= level:
            fallingInterpFunction = interp1d(np.array([nextPointValue, thisPointValue]), np.array([point+1, point]))
            interpPosition = fallingInterpFunction(level)

            falling.append(interpPosition)
            both.append(interpPosition)

    if mode is 'rising':
        return np.array(rising), len(rising)
    elif mode is 'falling':
        return np.array(falling), len(falling)
    else:
        return np.array(both), len(both)
